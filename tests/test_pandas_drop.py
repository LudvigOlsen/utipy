...
def test_drop_DF_columns():
    ...
    assert (droppedinf.columns == [
            'a', 'b', 'c', 's', 'na', 'zero', 'zeros']).all()
    ...
    assert (droppednone.columns == [
            'a', 'b', 'c', 's', 'na', 'infs', 'zero', 'zeros']).all()
    ...
    assert (droppedexc.columns == ['a', 'b', 'c',
            's', 'infs', 'zero', 'zeros']).all()
...
def test_drop_DF_rows():
    ...
    assert (droppedNaN.columns == ['a', 'b', 'c',
            's', 'na', 'infs', 'zero', 'zeros']).all()
    ...
    dropped0 = ut.drop(data=df,
                       axis=1,
                       value=0,
                       # more than one appearance of 0 (8 columns)
                       thresh=1.5 / 8,
                       direction='>')
